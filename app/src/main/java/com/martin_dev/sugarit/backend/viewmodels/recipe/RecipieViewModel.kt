...
import com.martin_dev.sugarit.backend.model.api.Spoonacular.recipies.nutrition.RecipieResponse
...
class RecipieViewModel : ViewModel() {
    ...
    fun translatedList(apiResults: List<RecipieResponse>, onAllTranslated: (List<RecipieResponse>) -> Unit)
    {
        ...
        {
            onAllTranslated(listOf(RecipieResponse(results = emptyList(), totalResults = 0)))
            return
        }
